...
const G4String& GateObjectRepeaterListMessenger::DumpMap() {
  static G4String theList = "linear cubicArray focusArray ring quadrant sphere genericRepeater";
  return theList;
}
//-------------------------------------------------------------------------------------------------------
...
void GateObjectRepeaterListMessenger::ListChoices()
{
  ...
  G4cout << "  focusArray:\n"
            "    Repeats the object using point coordinates loaded from ArrayRepeater_placements.txt\n"
            "    and rotates each copy so it points toward the configured focus position\n";
  G4cout << "  ring:\n"
...
void GateObjectRepeaterListMessenger::DoInsertion(const G4String& objectRepeaterTypeName)
{
  ...
  else if (objectRepeaterTypeName=="focusArray")
    newObjectRepeater = new GateFocusArrayRepeater(GetCreator(),insertionName);
