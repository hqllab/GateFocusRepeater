...
namespace
{
const G4double kPi = 3.141592653589793238462643383279502884197;
const G4double kTolerance = 1e-12;
const G4double kXSlice = 0.;
                                    ...
                                    const G4ThreeVector& targetPoint)
{
  ...
  if (currentDirection.mag2() <= kTolerance)
  ...
  if (std::abs(newPosition.y()) < 0.0001 && std::abs(newPosition.z()) < 0.0001) {
    if (dotProduct < 0.) {
      ...
      rotation.rotate(kPi, G4ThreeVector(0., 1., 0.));
...
bool ReadPlacementsFile(const G4String& filename, std::vector<FocusPlacement>& placements)
{
  ...
  if (!std::getline(inputFile, line)) {
    G4cerr << "Error: Empty placements file: " << filename << G4endl;
    return false;
  }

  std::istringstream headerStream(line);
  if (!(headerStream >> pointCount)) {
  ...
  for (G4int p = 0; p < pointCount; ++p) {
    ...
    G4double x, y, z;
    G4double tx, ty, tz;
    if (!(lineStream >> x >> y >> z >> tx >> ty >> tz)) {
      G4cerr << "Error: Invalid format for point " << p
             << " in file. Expected: x y z targetX targetY targetZ" << G4endl;
      return false;
    }

    FocusPlacement placement;
    placement.center = G4ThreeVector(x, y, z);
    placement.target = G4ThreeVector(tx, ty, tz);
                                              ...
                                              G4double)
{
  ...
  if (!ReadPlacementsFile(filename, placements))
    return;

  for (std::vector<FocusPlacement>::const_iterator it = placements.begin();
       it != placements.end(); ++it) {
    G4ThreeVector newPosition = it->center;
    const G4double focus = it->target.x();
    const G4double x = newPosition.x();

    if (std::abs(focus - x) > kTolerance) {
      const G4double ratio = (kXSlice - x) / (focus - x);
      newPosition = newPosition + ratio * (it->target - newPosition);
    }

    G4RotationMatrix rotation =
      BuildFocusRotation(currentRotationMatrix, newPosition, it->target);
...
void GateFocusArrayRepeater::DescribeMyself(size_t indent)
{
  ...
  G4cout << GateTools::Indent(indent) << "FileName:    " << m_placementsFilename << "\n";
}
